...
while read line
do
COUNTER=$[COUNTER + 1] && echo -ne "\\033[KPassword count [$COUNTER] Trying password [$line]\\r" && openssl aes-256-cbc -d -a -in $1 -pass pass:"$line" -out out.txt 2>out.txt >/dev/null
if file out.txt | grep -q 'out.txt: ASCII text'
	then
		echo "\n==================================================\n\n" && cat out.txt && echo "\n\n==================================================" && echo "\nThe password is $line\n"
		end_time="$(date -u +%s)"
		elapsed="$(($end_time-$start_time))"
		echo "Time took $elapsed seconds\n" && exit
	else
		:
fi
done < $2
